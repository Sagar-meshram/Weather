...
import { app, BrowserWindow, screen, shell, ipcMain, BrowserWindowConstructorOptions } from "electron"
...
function resetWindowPosition(): void {
  ...
  console.log("Resetting window position to default")
  const primaryDisplay = screen.getPrimaryDisplay()
  const workArea = primaryDisplay.workAreaSize
  
  // Set to center of screen
  const defaultX = Math.round((workArea.width - (state.windowSize?.width || 800)) / 2)
  const defaultY = 50 // Default Y position from top
  
  state.currentX = defaultX
  state.currentY = defaultY
  
  state.mainWindow.setPosition(defaultX, defaultY)
  console.log(`Window position reset to x: ${defaultX}, y: ${defaultY}`)
}
...
function loadEnvVariables() {
  if (isDev) {
    ...
    dotenv.config({ path: path.join((process as any).resourcesPath, ".env") })
  }
